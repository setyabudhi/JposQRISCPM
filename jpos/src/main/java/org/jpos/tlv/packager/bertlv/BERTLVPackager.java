...
public abstract class BERTLVPackager extends GenericPackager {
        ...
        try {
            ...
            while (consumed < tlvDataLength) {
                ...
                if (!nested && fld.length > 1 && (packager = fld[fld.length - 1]) != null &&
                    ...
                    fieldUnpackLogger(evt,tag, tlvSubFieldData);
                    evt.addMessage ("<unpack name=\""+tag+ "\">");
                    evt.addMessage ("</unpack>");
                    if (tag.equals("61")) {
                    	unpackSubTag(m, rawValueBytes, nested);
                    }
    ...
    public int unpackSubTag(ISOComponent m, byte[] b, boolean nested) throws ISOException {
        LogEvent evt = new LogEvent(this, "unpack");
        try {
            if (m.getComposite() == null)
                throw new ISOException("Can't call packager on non Composite");
            ...
            while (consumed < tlvDataLength) {
                ...
                if (!nested && fld.length > 1 && (packager = fld[fld.length - 1]) != null &&
                    ...
                    fieldUnpackLogger(evt,tag, tlvSubFieldData);
                    evt.addMessage ("<unpack subtagname=\""+tag+ "\">");
                    evt.addMessage ("</unpack>");
                    if (tag.equals("63")) {
                    	unpackSubSubTag(m, rawValueBytes, nested);
                    }
    ...
    public int unpackSubSubTag(ISOComponent m, byte[] b, boolean nested) throws ISOException {
        LogEvent evt = new LogEvent(this, "unpack");
        try {
            if (m.getComposite() == null)
                throw new ISOException("Can't call packager on non Composite");
            ...
            while (consumed < tlvDataLength) {
                ...
                if (!nested && fld.length > 1 && (packager = fld[fld.length - 1]) != null &&
                    ...
                    fieldUnpackLogger(evt,tag, tlvSubFieldData);
                    evt.addMessage ("<unpack subsubtagname=\""+tag+ "\">");
                    evt.addMessage ("</unpack>");
     ...
     */
    private static void fieldUnpackLogger(LogEvent evt, String tag , Object rawValueBytes) throws ISOException
    {
        evt.addMessage ("<unpack tag=\""+tag+ "\">");
       
        if (rawValueBytes instanceof ISOMsg) {
        	ISOMsg msg = (ISOMsg) rawValueBytes;
        	evt.addMessage ("  <value>"
   	                +ISOUtil.hexString((byte[])msg.getValue())
   	                + "</value>");
   	        evt.addMessage ("</unpack>");
   	        
        }
        else if (rawValueBytes instanceof ISOBinaryField) {
        	ISOBinaryField msg = (ISOBinaryField) rawValueBytes;
           	evt.addMessage ("  <value>"
   	                +ISOUtil.hexString((byte[])msg.getValue())
   	                + "</value>");
   	        evt.addMessage ("</unpack>");
        }
        else if  (rawValueBytes instanceof ISOField) {
        	ISOField msg = (ISOField) rawValueBytes;
           	evt.addMessage ("  <value>"
   	                +msg.getValue()
   	                + "</value>");
   	        evt.addMessage ("</unpack>");
